...
const getFuelKeyboard = (selected) => ({
    inline_keyboard: [
        ...fuelTypes.map((f) => [
            {
...
const getLocationInputKeyboard = () => new InlineKeyboard()
    ...
    .row()
    .text("🔙 Orqaga", "backToMenu");
export const addStation = async (ctx) => {
    try {
...
export const handleAddStationName = async (ctx) => {
    const step = ctx.session.step;
    const text = ctx.message?.text?.trim();
    if (!text)
        return ctx.reply("❌ Nom bo'sh bo'lishi mumkin emas!");
    if (step === "name") {
        ...
        await ctx.reply("🏢 Siz bu stansiyaning egasi yoki rahbari sifatida qo'shyapsizmi?\n\n" +
            "Iltimos agar siz shaxobcha ishchisi/egasi/rahbari bo'lmasangiz, shaxobcha ishchilariga bot haqida aytsangiz mamnun bolar edik!", { reply_markup: getOwnershipKeyboard() });
    }
};
export const handleStationLocation = async (ctx) => {
    ...
    ctx.session.step = "ownership";
    await ctx.reply(`✅ Stansiya joylashuvi saqlandi: ${latitude}, ${longitude}\n\n` +
        "🏢 Siz bu stansiyaning egasi yoki rahbari sifatida qo'shyapsizmi?\n\n" +
        "Iltimos agar siz shaxobcha ishchisi/egasi/rahbari bo'lmasangiz, shaxobcha ishchilariga bot haqida aytsangiz mamnun bolar edik!", { reply_markup: getOwnershipKeyboard() });
    return true;
};
export const handleStationCallbacks = async (ctx) => {
    ...
    if (data === "fuel_done") {
        ...
        if (!ctx.session.station.fuel_types ||
        ...
        await ctx.reply("📍 Stansiya joylashuvini yuboring:\n\n" +
    ...
    if (data === "station_share_location") {
        ...
        await ctx.editMessageText("📍 Stansiya joylashuvini yuboring:\n\n" +
            "📝 <b>Koordinatalarni yozing</b>: <code>41.3030, 69.2829</code>\n" +
            "📍 <b>Yoki lokatsiyani yuboring</b>", { parse_mode: "HTML" });
        return ctx.answerCallbackQuery();
    }
    if (data === "ownership_confirm") {
        ...
        try {
            ...
            if (existingStation) {
                ...
                return ctx.editMessageText(`❌ Ushbu joylashuvda (${location.lat}, ${location.lng}) allaqachon stansiya mavjud!\n\n` +
            ...
            await ctx.editMessageText(`✅ Stansiya ma'lumotlari yuborildi va ko'rib chiqilish uchun navbatga qo'shildi:\n\n` +
                `🏷️ <b>Nomi:</b> ${name}\n` +
            ...
            for (const adminId of ADMINS) {
                try {
                    await ctx.api.sendMessage(adminId, "🆕 Yangi stansiya qo‘shilishga yuborildi, iltimos menyuga o‘tib tekshiring.", {
                        reply_markup: {
                            inline_keyboard: [
    ...
    if (data === "ownership_deny") {
        ...
        try {
            ...
            await ctx.editMessageText(`✅ Rahmat! Ma'lumotlaringiz ko'rib chiqilish uchun yuborildi.\n\n` +
