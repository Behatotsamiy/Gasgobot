...
export async function Stationlong(station, ctx) {
...
export async function editStation(ctx, id, call) {
...
function showFuelSelectionMenu(ctx, selectedFuels) {
    ...
    const getEditFuelKeyboard = (selected) => ({
        inline_keyboard: [
            ...fuelTypes.map((fuel) => [
                {
...
export async function handleEditFuelSelection(ctx, fuelType) {
...
export async function handleFuelDone(ctx) {
    ...
    if (!user)
        return ctx.reply("❌ Foydalanuvchi topilmadi.");
    const updated = await StationModel.findByIdAndUpdate(ctx.session.editingStationId, { fuel_types: ctx.session.station.fuel_types }, { new: true });
    if (!updated)
...
export async function handleStationNameUpdate(ctx, newName) {
    const user = await UserModel.findOne({ telegramId: ctx.from?.id });
    if (!user)
        return ctx.reply("❌ Foydalanuvchi topilmadi.");
    const updated = await StationModel.findByIdAndUpdate(ctx.session.editingStationId, { name: newName }, { new: true });
    if (!updated)
...
export async function handleStationLocationUpdate(ctx, locationInput) {
    let lat, lng;
    ...
    if (!user)
        return ctx.reply("❌ Foydalanuvchi topilmadi.");
    const updated = await StationModel.findByIdAndUpdate(ctx.session.editingStationId, { location: { lat, lng } }, { new: true });
    if (!updated)
